...
class _OutlookCalendarPageState extends State<OutlookCalendarPage> {
  ...
  void _addTaskForDate(DateTime startTime) async {
    ...
    if (confirmed == true) {
      ...
      print("Envoi HTTP...");
      final success = await api.sendTaskToServer(task);
      print("Statut HTTP : $success");

      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
            success
                ? "✅ Tâche envoyée avec succès !"
                : "⚠️ Enregistrement local seulement.",
          ),
              ...
              : ListView.builder(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    return ListTile(
                      ...
                      subtitle: Text(
                        task.modificationType.toString().split('.').last,
                      ),
                      trailing: Checkbox(
